  ...
  root.innerHTML = `
    <section class="serenity-hero shell">
      <div class="serenity-hero-copy">
        <div class="serenity-identity"><img src="${new URL(app.icon, import.meta.url).href}" width="60" height="60" alt="YNM图标"><div><span class="kicker">A LITTLE QUIET, EVERY DAY</span><p>YNM <span> / ${escapeHtml(app.platform)}</span></p></div></div>
    ...
    <section class="serenity-start shell" id="start" aria-labelledby="start-title"><div><p class="kicker">YOUR FIRST QUEST</p><h2 id="start-title">三步，<br>开启你的封印之旅。</h2><p>无需账号，也没有复杂的任务。<br>从一个容易让你分心的应用开始。</p></div><ol><li><span>壹</span><div><h3>授予屏幕使用时间权限</h3><p>在主页完成个人授权，让YNM可以为你隐藏所选应用。</p></div></li><li><span>贰</span><div><h3>把分心的应用收进笼中</h3><p>点击锁妖笼，使用系统选择器挑选应用并保存。锁定时，它们会保持隐藏。</p></div></li><li><span>叁</span><div><h3>需要时，再短暂解封</h3><p>选一个合适的时长，或通过桌面组件快捷操作。也可以随时提前重新封印。</p></div></li></ol></section>
    <aside class="serenity-boundary shell"><span aria-hidden="true">◇</span><div><h2>是日常的帮手，不是强制的枷锁。</h2><p>YNM面向个人自我管理，不提供家长控制。恢复锁定依赖系统调度，持续锁屏时可能延迟；“自动”模式在解封半小时后等待后台机会，并非精确定时。游戏里的封印进度，不改变真实应用限制。</p></div></aside>
